...
void doit(int fd) {
    ...
    // sscanf는 sacnf와 유사하지만, 입력을 표준 입력이 아닌 문자열에서 읽어옴
    // 반환값으로 성공적으로 읽어들인  항목의 개수 반환
    sscanf(buf, "%s %s %s", method, uri, version); // 요청 라인에서 메서드, URI, 버전을 추출

    printf("***************************************\n");
    printf("tiny server HTTP version : %s\n", version);
    printf("***************************************\n");
    fflush(stdout);


    /* Step 2: HTTP 메서드 확인 */
    // Tiny 서버는 GET 메서드만 지원하므로, 다른메서드가 요청될 경우 오류 메시지를 반환하고 종료
...
void serve_dynamic(int fd, char *filename, char *cgiargs) {
    ...
    if (Fork() == 0) {             // 자식 프로세스 생성, 자식 프로세스가 실행
        setenv("QUERY_STRING", cgiargs, 1);       // CGI 인수를 환경 변수 QUERY_STRING에 설정
        Dup2(fd, STDOUT_FILENO);                  // 자식 프로세스의 표준 출력을 클라이언트 소켓으로 리다이렉션
